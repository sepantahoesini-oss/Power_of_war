...
async function insertUser(env,d){
  ...
  if(cols.has('loyalty_date')){fields.push('loyalty_date');vals.push(d.loyalty_date||null);}
...
async function unblock(env,code){
  ...
  if(!bc)throw new Error('ستون وضعیت مسدودی در users وجود ندارد');
  await env.DB.prepare(`UPDATE users SET ${bc}=NULL,blocked_until=NULL WHERE code=?`).bind(code).run();
}
...
async function getPlayerByCode(env,code){
  return env.DB.prepare(`
    SELECT sp.*,u.name,u.total_games,u.loyalty_date,u.blocked_until,u.block_type,a.*
    FROM s8_players sp
...
async function addIncome(env){
  ...
  const claim=await env.DB.prepare(`
    INSERT INTO s8_settings(key,value) VALUES('last_income_day',?)
  ...
  async fetch(req,env){
    ...
    try{
      ...
      if(path==='/api/season/buy'&&req.method==='POST'){
        ...
        if(!p)return json({message:'بازیکن پیدا نشد.'},404);
        const startingDollars=Number(p.dollars||0);
        if(startingDollars<total)return json({message:'موجودی کافی نیست'},400);
        const remaining=startingDollars-total;
        const code=txCode();
        const stm=[env.DB.prepare(`UPDATE s8_players SET dollars=dollars-?,updated_at=CURRENT_TIMESTAMP WHERE id=? AND dollars=? AND dollars>=?`).bind(total,p.id,startingDollars,total)];
        if(meta.mode==='income'){
          stm.push(env.DB.prepare(`INSERT INTO s8_income_assets(player_id,kind,qty) SELECT ?,?,? WHERE EXISTS(SELECT 1 FROM s8_players WHERE id=? AND dollars=?) ON CONFLICT(player_id,kind) DO UPDATE SET qty=qty+excluded.qty`).bind(p.id,kind,qty,p.id,remaining));
        }else{
          stm.push(env.DB.prepare(`UPDATE s8_assets SET ${kind}=${kind}+? WHERE player_id=? AND EXISTS(SELECT 1 FROM s8_players WHERE id=? AND dollars=?)`).bind(qty,p.id,p.id,remaining));
        }
        stm.push(env.DB.prepare(`INSERT INTO s8_transactions(tx_code,player_id,user_code,type,description,amount) SELECT ?,?,?,?,?,? WHERE EXISTS(SELECT 1 FROM s8_players WHERE id=? AND dollars=?)`).bind(code,p.id,p.user_code,'purchase',`خرید ${meta.name} × ${qty}`,total,p.id,remaining));
        const results=await env.DB.batch(stm);
        if(Number(results?.[0]?.meta?.changes||0)!==1)return json({message:'موجودی در لحظه خرید تغییر کرد، دوباره تلاش کنید.'},409);
        return json({message:'پرداخت موفق',transaction_id:code});
      }

      if(path==='/api/season/buy-batch'&&req.method==='POST'){
        const a=await authPlayer(req,env);if(a.error)return a.error;
        const d=await req.json(),items=Array.isArray(d.items)?d.items:[];
        if(!items.length)return json({message:'سبد خرید خالی است.'},400);
        const normalized=[];let total=0;
        for(const item of items){
          const kind=esc(item.kind||item.key),qty=n(item.quantity),meta=ASSET_META[kind];
          if(!meta||!Number.isSafeInteger(qty)||qty<1)return json({message:'یکی از آیتم‌های خرید نامعتبر است.'},400);
          const cost=meta.price*qty;if(!Number.isSafeInteger(cost))return json({message:'مبلغ خرید نامعتبر است.'},400);
          total+=cost;normalized.push({kind,qty,cost,meta});
        }
        if(!Number.isSafeInteger(total))return json({message:'مبلغ نهایی نامعتبر است.'},400);
        const p=await getPlayerById(env,a.player.id);if(!p)return json({message:'بازیکن پیدا نشد.'},404);
        const startingDollars=Number(p.dollars||0);if(startingDollars<total)return json({message:'موجودی کافی نیست'},400);
        const remaining=startingDollars-total,stm=[env.DB.prepare(`UPDATE s8_players SET dollars=dollars-?,updated_at=CURRENT_TIMESTAMP WHERE id=? AND dollars=? AND dollars>=?`).bind(total,p.id,startingDollars,total)];
        for(const item of normalized){
          if(item.meta.mode==='income')stm.push(env.DB.prepare(`INSERT INTO s8_income_assets(player_id,kind,qty) SELECT ?,?,? WHERE EXISTS(SELECT 1 FROM s8_players WHERE id=? AND dollars=?) ON CONFLICT(player_id,kind) DO UPDATE SET qty=qty+excluded.qty`).bind(p.id,item.kind,item.qty,p.id,remaining));
          else stm.push(env.DB.prepare(`UPDATE s8_assets SET ${item.kind}=${item.kind}+? WHERE player_id=? AND EXISTS(SELECT 1 FROM s8_players WHERE id=? AND dollars=?)`).bind(item.qty,p.id,p.id,remaining));
          const code=txCode();
          stm.push(env.DB.prepare(`INSERT INTO s8_transactions(tx_code,player_id,user_code,type,description,amount) SELECT ?,?,?,?,?,? WHERE EXISTS(SELECT 1 FROM s8_players WHERE id=? AND dollars=?)`).bind(code,p.id,p.user_code,'purchase',`خرید ${item.meta.name} × ${item.qty}`,item.cost,p.id,remaining));
        }
        const results=await env.DB.batch(stm);
        if(Number(results?.[0]?.meta?.changes||0)!==1)return json({message:'موجودی در لحظه خرید تغییر کرد، دوباره تلاش کنید.'},409);
      ...
      if(path==='/api/season/transfer'&&req.method==='POST'){
        ...
        // Atomic sender debit: concurrent requests cannot spend the same balance twice.
        if(kind==='dollars'){
          stm.push(env.DB.prepare('UPDATE s8_players SET dollars=dollars-? WHERE id=? AND dollars>=?').bind(qty,p.id,qty));
          stm.push(env.DB.prepare('UPDATE s8_players SET dollars=dollars+? WHERE id=?').bind(qty,receiver.id));
        }else if(kind==='oil'){
          stm.push(env.DB.prepare('UPDATE s8_players SET oil=oil-? WHERE id=? AND oil>=?').bind(qty,p.id,qty));
          stm.push(env.DB.prepare('UPDATE s8_players SET oil=oil+? WHERE id=?').bind(qty,receiver.id));
        }else{
          stm.push(env.DB.prepare(`UPDATE s8_assets SET ${kind}=${kind}-? WHERE player_id=? AND ${kind}>=?`).bind(qty,p.id,qty));
          stm.push(env.DB.prepare(`UPDATE s8_assets SET ${kind}=${kind}+? WHERE player_id=?`).bind(qty,receiver.id));
        }
        ...
        if(Number(results?.[0]?.meta?.changes||0)!==1){
          return json({message:'موجودی در لحظه انتقال تغییر کرده است، دوباره تلاش کنید.'},409);
        }
      ...
      if(path==='/api/season/battle'&&req.method==='POST'){
        ...
        let defender=null;if(d.to_country)defender=await getPlayerByCountry(env,esc(d.to_country));else defender=await getPlayerByCode(env,esc(d.to_code).toUpperCase());
        if(!defender||defender.id===a.player.id)return json({message:'کشور حریف معتبر نیست.'},400);
        const allowed=mode==='defense'?DEFENSE_KEYS:WAR_KEYS,selected=d.assets||{},clean=[];
        for(const [key,value] of Object.entries(selected)){const qty=n(value);if(qty>0){if(!allowed.includes(key))return json({message:'این تجهیز برای این نوع نبرد مجاز نیست.'},400);if(Number(a.player[key]||0)<qty)return json({message:`تعداد ${ASSET_META[key]?.name||key} کافی نیست.`},400);clean.push([key,qty]);}}
        if(!clean.length)return json({message:'حداقل یک تجهیز انتخاب کنید.'},400);
        const code=txCode(),conditions=clean.map(([k,q])=>`${k}>=?`).join(' AND '),binds=clean.flatMap(([k,q])=>[q]);
        const decrement=await env.DB.prepare(`UPDATE s8_assets SET ${clean.map(([k,q])=>`${k}=${k}-?`).join(',')} WHERE player_id=? AND ${conditions}`).bind(...clean.map(x=>x[1]),a.player.id,...binds).run();
        if(Number(decrement.meta?.changes||0)!==1)return json({message:'تجهیزات در لحظه ثبت جنگ تغییر کرده‌اند، دوباره تلاش کنید.'},409);
        try{
          const battle=await env.DB.prepare('INSERT INTO s8_battles(code,attacker_player_id,defender_player_id,mode,scenario) VALUES(?,?,?,?,?) RETURNING id').bind(code,a.player.id,defender.id,mode,scenario).first();
          if(!battle)throw new Error('ثبت سناریو انجام نشد');
          await env.DB.batch(clean.map(([key,qty])=>env.DB.prepare('INSERT INTO s8_battle_assets(battle_id,asset_key,quantity) VALUES(?,?,?)').bind(battle.id,key,qty)));
          return json({message:'سناریو ارسال شد',code});
        }catch(e){
          await env.DB.batch(clean.map(([key,qty])=>env.DB.prepare(`UPDATE s8_assets SET ${key}=${key}+? WHERE player_id=?`).bind(qty,a.player.id)));
          throw e;
        }
      }
