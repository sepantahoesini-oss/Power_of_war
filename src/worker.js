        ...
        for(
          ...
          if(
            ...
            return json(
              {
                message:
                  'یکی از آیتم‌های خرید نامعتبر است.'
              },
              400
            );
  ...
  for(
    ...
    if(
      ...
      return json(
        {
          message:
            `موجودی ${META[item.key]?.name||item.key} کافی نیست.`
        },
        400
      );
  ...
  const rows=
    await env.DB
      .prepare(`
        SELECT
          b.*,
          ap.country attacker_country,
          dp.country defender_country
        FROM s8_battles b
