...
export default {
  ...
  async fetch(req,env){
    ...
    try{
      ...
      if(path==='/api/admin/bitcoin'&&req.method==='POST'){const price=n((await req.json()).price);if(!Number.isSafeInteger(price)||price<1)return json({message:'قیمت نامعتبر است.'});await setSetting(env,'bitcoin_price',price);return json({message:'قیمت بیت‌کوین ثبت شد.'});}
